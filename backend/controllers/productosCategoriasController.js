...
const obtenerProductosCategorias = (req, res) => {
  db.query('SELECT pc.id_producto_categoria, p.nombre AS nombre_producto, c.nombre AS nombre_categoria , p.imagen AS imagen_categoria , precio , stock  FROM productos_categorias pc INNER JOIN productos p ON pc.id_producto = p.id_producto INNER JOIN categorias c ON pc.id_categoria = c.id_categoria', (error, resultados) => {
    if (error) {
...
const obtenerProductoCategoriaPorId = (req, res) => {
  ...
  db.query('SELECT pc.id_producto_categoria,descripcion, p.nombre AS nombre_producto, c.nombre AS nombre_categoria , p.imagen AS imagen_categoria , precio , stock  FROM productos_categorias pc INNER JOIN productos p ON pc.id_producto = p.id_producto INNER JOIN categorias c ON pc.id_categoria = c.id_categoria WHERE pc.id_producto_categoria = ?', [id], (error, resultados) => {
    if (error) {
